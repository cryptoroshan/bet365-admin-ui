...
const ModalLeagueTransalte = ({ item }: { item: any }) => {
  const { isLeagueTranslateModalOpen, closeLeagueTranslateModal } = useModalContext();

  return (
    <Modal
      title={item?.league_default_name}
      open={isLeagueTranslateModalOpen}
      onCancel={closeLeagueTranslateModal}
      footer={[
        <div key="Save" className="flex gap-2 justify-center">
          <button
            className="px-4 py-1.5 rounded-md bg-[#333] text-brand-light-grey"
            onClick={closeLeagueTranslateModal}
          >
            Save
          </button>
        </div>,
...
export default ModalLeagueTransalte;
